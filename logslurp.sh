...
if [[ -z $1 ]] || [[ -z $2 ]] || [[ -z $3 ]] || [[ -z $4 ]]; then
    echo "Usage: $0 adminuser@master adminuser@windowsnode keyFile outputPath"
    echo
...
ssh -v -i $3 -t -o UserKnownHostsFile=/dev/null -o "StrictHostKeyChecking=no" -o "ProxyCommand ssh -i $3 -o StrictHostKeyChecking=no -W %h:%p $1" $2 powershell.exe "-nologo -command \" Invoke-Expression -Command ([System.Text.Encoding]::ASCII.GetString([System.Convert]::FromBase64String('$remoteEncoded')))  \" " | tee "$tempOutput"
remoteFile=$(cat -v "$tempOutput" | grep -o "^C:.*zip" | tail -n1)
scp -v -i $3 -o UserKnownHostsFile=/dev/null -o "StrictHostKeyChecking=no" -o "ProxyCommand ssh -i $3 -o StrictHostKeyChecking=no -W %h:%p $1" $2:"$remoteFile" $4
rm "$tempOutput"
